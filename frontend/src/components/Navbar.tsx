...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full">
      ...
      <div className="relative overflow-hidden bg-primary text-white">
        ...
        <div className="container relative flex h-9 items-center justify-between">
          <div className="hidden sm:flex items-center gap-1.5 text-[11px] font-jost font-semibold">
            ...
            <span className="text-white/90">#1 Rated Tech Store in Kenya</span>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-4">
            {isAuthenticated ? (
              <button onClick={handleLogout} className="text-[11px] font-jost text-white/70 hover:text-white transition-colors">
                Logout
              </button>
            ) : (
              <Link to="/login" className="text-[11px] font-jost text-white/70 hover:text-white transition-colors">Login</Link>
            )}
            <Link to="/contact" className="text-[11px] font-jost text-white/70 hover:text-white transition-colors">Support</Link>
          </div>
        </div>
      </div>
        ...
        <div className="container flex h-[68px] items-center justify-between gap-4">
          <Link to="/" className="group flex shrink-0 items-center gap-2.5">
            ...
            <div className="hidden sm:block">
              <span className="block font-bodoni text-[17px] font-bold leading-none tracking-tight text-primary">
                Gadgets Planet
              </span>
          ...
          <form onSubmit={handleSearch} className="hidden md:block flex-1 max-w-[460px] relative">
          ...
          <div className="flex items-center gap-1">
            ...
            <Link to={isAuthenticated ? '/' : '/login'} className="hidden sm:flex h-10 w-10 items-center justify-center rounded-xl text-primary hover:bg-grey-light transition-colors">
      ...
      {/* Category nav */}
      <div className="hidden md:block border-b border-grey-mid/60 bg-white/50 backdrop-blur-sm">
              ...
              <Link
                ...
                className={`font-jost text-[13px] font-medium px-4 py-2 rounded-full whitespace-nowrap transition-all ${
                  location.pathname === link.path
                    ...
                    : 'text-secondary hover:text-primary hover:bg-grey-light'
                }`}
              >
                {link.name}
              </Link>
      ...
      {/* Mobile menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-b border-grey-mid shadow-card animate-in slide-in-from-top duration-200">
          <div className="container py-5 space-y-5">
            <form onSubmit={handleSearch} className="relative">
              ...
              <input
                ...
                className="w-full h-11 bg-grey-light border border-grey-mid rounded-2xl pl-11 pr-4 text-[13px] outline-none focus:border-cta/50"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
            </form>
            <nav className="grid grid-cols-2 gap-2">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
                  className="font-jost text-[13px] font-medium py-3 px-4 rounded-xl bg-grey-light hover:bg-primary hover:text-white transition-all text-center"
                >
                  {link.name}
                </Link>
              ))}
            ...
            <div className="flex gap-3 pt-2 border-t border-grey-mid">
              {isAuthenticated ? (
                <button onClick={handleLogout} className="flex-1 py-2.5 text-[13px] font-jost font-medium border border-grey-mid rounded-xl hover:bg-grey-light transition-colors">
                  Logout
                </button>
              ) : (
                <Link to="/login" className="flex-1 py-2.5 text-center text-[13px] font-jost font-medium border border-grey-mid rounded-xl hover:bg-grey-light transition-colors">
                  Login
                </Link>
              )}
              <Link to="/contact" className="flex-1 py-2.5 text-center text-[13px] font-jost font-medium bg-primary text-white rounded-xl hover:bg-cta transition-colors">
                Support
              </Link>
            </div>
          </div>
